...
do
    if [ "$line" = "-" ]; then
        ...
        scp  /spark-examples/setup.py root@$line:/spark-examples/ && scp /spark-examples/manager root@$line:/spark-examples/ && scp /spark-examples/workers root@$line:/spark-examples/
        echo "Setup $line"
