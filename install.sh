...
copy_payload() {
  ...
  # Copy Spark tarball if available locally
  if [ -f "$BASE_DIR/spark-4.1.1-bin-hadoop3.tgz" ]; then
    scp $SSH_OPTS "$BASE_DIR/spark-4.1.1-bin-hadoop3.tgz" root@"$ip":"$BASE_DIR/"
  fi
...
for ip in $WORKER_IPS; do
  ...
  ssh $SSH_OPTS root@"$ip" "test -x /usr/local/spark/bin/pyspark"
  echo "[OK] $ip"
done

echo "Spark 4.1.1 deployment completed successfully."
